...
/**
 * Initializes and configures 2D margin grid shapes rendered directly inside the background canvas (.bg-rift).
 * This method selectively registers shapes on the home page context.
 *
 * @return {void}
 */
export function init2DGridCanvas() {
    ...
    // Hook into background canvas render loop
    window.bgOverlayRenderers = window.bgOverlayRenderers || [];
    window.bgOverlayRenderers.push(({ ctx, width, height, step }) => {
        const GRID_STEP = step || 45;
        ...
        if (width < 768 || marginWidth < GRID_STEP) return;
